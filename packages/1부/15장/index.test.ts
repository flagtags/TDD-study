...
import { Money, Bank, Sum, Expression } from './index';
...
test('Sum.plus 테스트', ()=>{
  ...
  const tenFranc: Expression = Money.franc(10);
  const sum = fiveDollar.plus(tenFranc)
  const sumAgain = sum.plus(tenFranc);

  const bank = new Bank();
  bank.addRate('CHF', 'USD', 2);
  const result = bank.reduce(sumAgain,'USD');
...
test('Sum.times 테스트', () => {
  ...
  const tenFranc: Expression = Money.franc(10);
  const sum = fiveDollar.plus(tenFranc)
  const sumTimes = sum.times(2);

  const bank = new Bank();
  bank.addRate('CHF', 'USD', 2);
  const result = bank.reduce(sumTimes,'USD');
